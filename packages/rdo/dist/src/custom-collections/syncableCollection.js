    ...
    constructor({ makeCollectionKeyFromSourceElement, makeCollectionKeyFromRdoElement, makeRdo, } = {}) {
        ...
        this.insertElement = (key, value) => {
            if (!this._map$.has(key)) {
                ...
                __1.CollectionUtils.Array.insertElement({ collection: this._array$, key, value });
                return true;
            }
            else
                return false;
        ...
        this.updateElement = (key, value) => {
            if (this.makeCollectionKeyFromRdoElement) {
                if (!this._map$.has(key)) {
                    this._map$.set(key, value);
                    __1.CollectionUtils.Array.updateElement({ collection: this._array$, makeCollectionKey: this.makeCollectionKeyFromRdoElement, value });
                    return true;
                }
                else
                    return false;
        ...
        this.clearElements = () => {
            this._map$.clear();
            return __1.CollectionUtils.Array.clear({ collection: this._array$ });
        };
