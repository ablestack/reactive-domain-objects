...
const relayOperationOptimizer = require('@graphql-toolkit/relay-operation-optimizer');
...
function generateImportStatement(statement) {
    ...
    const importNames = importSource.identifiers && importSource.identifiers.length ? `{ ${importSource.identifiers.join(', ')} }` : '*';
...
class ClientSideBaseVisitor extends BaseVisitor {
    ...
    _generateFragment(fragmentDocument) {
        ...
        const isDocumentNode = this.config.documentMode === exports.DocumentMode.documentNode ||
            ...
            case exports.DocumentMode.external: {
                if (this._collectedOperations.length > 0) {
                    if (this.config.importDocumentNodeExternallyFrom === 'near-operation-file' && this._documents.length === 1) {
                        imports.push(`import * as Operations from './${this.clearExtension(path.basename(this._documents[0].location))}';`);
                    }
                    else {
...
function optimizeOperations(schema, documents) {
    const newDocuments = relayOperationOptimizer.optimizeDocuments(schema, documents.map(s => s.document));
