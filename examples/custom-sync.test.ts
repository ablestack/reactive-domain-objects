import { GraphSynchronizer, IContinueSmartSync, ICustomSync, IMakeRdo, IRdoNodeWrapper } from '@ablestack/rdo';
...
export class CustomSyncBarRDO implements ICustomSync<Bar> {
  ...
  public synchronizeState({ sourceObject, continueSmartSync }: { sourceObject: Bar; continueSmartSync: IContinueSmartSync }) {
    console.log(' ---> in CustomSyncBarRDO-synchronizeState');

    this.id = `custom-id-${sourceObject.id}`;

    // sync bazzz
    console.log(' ---> in CustomSyncBarRDO-synchronizeState A', { sourceNodeItemKey: 'baz', sourceItemValue: sourceObject, rdoNodeItemKey: 'bazzz', rdoNodeItemValue: this.bazzz });
    continueSmartSync({ sourceNodeItemKey: 'baz', sourceItemValue: sourceObject, rdoNodeItemKey: 'bazzz', rdoNodeItemValue: this.bazzz });

    // sync mapOfBazz
    console.log(' ---> in CustomSyncBarRDO-synchronizeState B', { sourceNodeItemKey: 'mapOfBaz', sourceItemValue: sourceObject, rdoNodeItemKey: 'mapOfBazzz', rdoNodeItemValue: this.mapOfBazzz });
